...
/**
 * Utility class for calendar and date operations
 * Provides methods for working with day indices, dates, and formatting
 */
export class CalendarUtils {
  ...
  static getTodayIndex(year: number): number {
    ...
    if (now.getFullYear() !== year) {
      return now.getFullYear() < year ? -1 : CALENDAR.DAYS_IN_YEAR;
    }
