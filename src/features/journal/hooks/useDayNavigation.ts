...
/**
 * Hook for keyboard navigation between days
 * Handles arrow key navigation and Enter key to open editor
 * 
 * @param {UseDayNavigationOptions} options - Navigation options
 * @param {number} options.year - Year for navigation
 ...
 * @returns {Function} setSelectedDayIndex - Set the selected day index
 */
export function useDayNavigation({
  ...
  // Initialize to today if enabled - only run once when enabled changes
  const hasInitialized = useRef(false);
  useEffect(() => {
    if (enabled && selectedDayIndex === null && !hasInitialized.current) {
  ...
  const navigateToDay = useCallback(
    (dayIndex: number) => {
      if (dayIndex < 0 || dayIndex >= CALENDAR.DAYS_IN_YEAR) return;
  ...
  const navigateDown = useCallback(() => {
    ...
    const newIndex = selectedDayRef.current + 7;
    if (newIndex < CALENDAR.DAYS_IN_YEAR) {
      navigateToDay(newIndex);
    }
  }, [navigateToDay]);
